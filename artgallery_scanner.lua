-- ArtGallery scanner: finds reference-worthy images inside an EPUB.
--
...
-- ── CBZ / ZIP-of-images scan ───────────────────────────────────────────────
-- Scans a comic archive (.cbz / .zip of page images) via an open
-- Archiver.Reader. Enumerates image entries, natural-sorts them (comic page
-- order), and returns the same { images = {...} } shape as scan() so the
-- viewer can consume it unchanged. Dimensions are left at 0: the gallery
-- does not need them, and sniffing every page would mean re-seeking the
-- archive per image (slow on-device). `arc` MUST be an open Archiver.Reader.
function M.scan_cbz(arc)
    if not arc then return nil, "no_archive" end
    local names = {}
    for entry in arc:iterate() do
        if entry and entry.mode == "file" then
            local p = entry.path:lower()
            if p:match("%.jpe?g$") or p:match("%.png$") or p:match("%.gif$")
               or p:match("%.webp$") or p:match("%.svg$") or p:match("%.bmp$") then
    ...
    table.sort(names, function(a, b)
        local function pad(s)
            return (s:gsub("%d+", function(n)
                return string.format("%05d", tonumber(n) or 0)
            end))
        end
        return pad(a) < pad(b)
    end)
    ...
    for i, name in ipairs(names) do
        -- 标记 SVG，使 _render 走 crengine SVG 渲染（MuPDF 无法直接渲染 SVG）。
        -- 误判安全：_render 先试 SVG、失败会自动回退 renderImageData。
        local fmt = name:lower():match("%.svg$") and "svg" or nil
        images[#images + 1] = {
            path = name,
            raw_path = name,
            spine_index = i,
