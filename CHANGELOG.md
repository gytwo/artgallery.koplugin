...
## 阶段三十 / Phase 30 — 「关于 美术馆」说明文档为菜单项配图标（轻量 Unicode 方案）· v1.0.14
- **中文**：v1.0.14。用户要求「关于 美术馆」说明文档中为各菜单项配上其在菜单中对应的真实图标；并明确「凡是有图标的菜单，说明文档中就一定要配上正确图标」。经核实：菜单图标实为插件 `assets/*.svg`（gallery/mode/rotate/navigate/restore/reset-rotation…）与角标 `eyeclosed.svg`（睁眼加斜杠），而 TextViewer 用 `x_smallinfofont`=NotoSans-Regular.ttf 渲染，无法内联 SVG、也不渲染彩色 emoji（KPW3 缺 emoji 字体易变方框）。故采用轻量方案（用户确认可接受）：给每个带图标的菜单项配 Noto Sans 收录的字形——⤢ 全屏查看、▦ 图库、◎ 模式（防剧透范围，图标为书本/范围形）、↻ 旋转 90° / ↺ 重置旋转、➤ 在书中定位、↩ 恢复被忽略、☑ 三个开关项（菜单中以勾选框标记）；长按菜单的「收藏图片」配 ⭐。「划掉眼睛」此前为唯一缺口：既无对应 Unicode，其近似 emoji 👁 又不在 Noto Sans（KPW3 上易变方框）。最终按用户决定：在眼睛行保留文字「划掉的眼睛」的同时前加眼睛 emoji 👁 作图标占位（「👁 划掉的眼睛」），由用户自行在 KPW3 安装 emoji 字体并让 KOReader 字体回退生效后方能渲染；未装则显示方框但文字仍表意。已确认「清除收藏」在子菜单无 `icon` 字段、属纯文字项，未强行配图标。语法校验 SYNTAX OK。注：本改动仅改说明文档字符串，不影响 BLC 抑制兜底（指南仍含「关闭」+「书」组合，v1.0.13 的恢复逻辑继续生效）。
- **English**: v1.0.14. User asked the "About ArtGallery" doc to show each menu item's real icon; and clarified "any menu that has icons must show the correct icon in the doc". Finding: menu icons are plugin `assets/*.svg` (gallery/mode/rotate/navigate/restore/reset-rotation…) and the corner badge `eyeclosed.svg` (eye with slash); TextViewer renders with `x_smallinfofont`=NotoSans-Regular.ttf, which cannot inline SVGs nor draw color emoji (KPW3 lacks an emoji font, would show a box). So a lightweight approach (user confirmed acceptable): prefix each icon-bearing menu item with a Noto Sans glyph — ⤢ fullscreen, ▦ gallery, ◎ mode (scope/anti-spoiler; icon is a book/range shape), ↻ rotate 90° / ↺ reset rotation, ➤ locate-in-book, ↩ restore-ignored, ☑ the three toggles (checkboxes in the menu); long-press "favorite" gets ⭐. The only item that can't be faithfully shown is the "crossed-out eye": no Unicode equivalent, and its emoji approximation 👁 isn't in Noto Sans (box on KPW3), so per the user-accepted "sole gap" it stays as text. Confirmed "清除收藏" has no `icon` field (plain-text submenu entry) and was left as-is. SYNTAX OK. Note: this only edits doc strings; the BLC-suppression fallback is untouched (guide still contains "关闭"+"书", so v1.0.13's recovery keeps working).

## 阶段二十九 / Phase 29 — 根治「关于 美术馆」白闪/无弹窗（锁定真凶为 BookLoadCover Plus 文本误判）（2026-08-11）
- **中文**：v1.0.13。用户在 v1.0.12 真机复测反馈「弹窗问题依然如旧」——连续第六轮被证伪。吸取 v1.0.12 误删取证埋点的教训，重新注入带 `debug.traceback()` 的强诊断埋点，经 KPW3 回传 crash.log 彻底锁定真凶：根本不是本插件弹窗逻辑，而是同机安装的 BookLoadCover Plus 补丁（patches/2-bookloadcover-plus.lua）。其 `patchUIManagerShow`（:1203-1219）在 `shouldSuppressClosingNotice()` 为真时，对文本同时含「关闭」与「书」的 widget 调用 `textLooksLikeClosingBookNotice` 命中（中文判定 `{"关闭","书"}`，:486），误判为「关闭书籍」弹窗，将 widget 的 `paintTo` 清空为 `function() end`、`visible=false`、`hasVisibleContent=false` 静默吞掉——`onShow` 仍触发但 `paintTo` 永不调用，故屏幕上看不到弹窗（白闪/无弹窗）。本「关于」长文本恰好同时含这两字（「关闭全部」「在书中定位/跨书本」），命中误判。这推翻了阶段二十八「残留 tap 误关」的结论：`paintTo` 被清空后弹窗本就从不绘制，且日志显示本次 `onTapClose` 是 5 秒后的真实 tap 而非残留手势。早期 InfoMessage 短文本能弹（不含该组合）、poker24 不闪（文本未命中）均由此厘清。修复：移除 v1.0.12 的延迟/TapClose 吸收/全部诊断埋点；在 `UIManager:show(tv)` 后检测 `_bookloadcover_suppressed` 标记，恢复 `paintTo`（从 `_original_paintTo_bookloadcover`）、`visible`、`hasVisibleContent` 并 `setDirty` 强制重绘（防御性：仅当原 paintTo 为函数时恢复；未装该补丁时分支跳过，正常显示）。语法校验 SYNTAX OK。
- **English**: v1.0.13. User reported v1.0.12's About popup still broken — the 6th consecutive real-device refutation. Re-injecting `debug.traceback()` diagnostics (after v1.0.12 wrongly removed them) and reading the KPW3 crash.log finally pinned the true culprit: not our popup logic, but the co-installed BookLoadCover Plus patch (patches/2-bookloadcover-plus.lua). Its `patchUIManagerShow` (:1203-1219), when `shouldSuppressClosingNotice()` is true, calls `textLooksLikeClosingBookNotice` on any widget whose text contains both "关闭" and "书" (Chinese rule `{"关闭","书"}`, :486), misjudges it as the "closing book" notice, and zeroes its `paintTo` to `function() end`, sets `visible=false`/`hasVisibleContent=false` — `onShow` still fires but `paintTo` never runs, so the popup is never drawn (the white flash / no-popup). Our About text happens to contain both words ("关闭全部", "在书中定位"/"跨书本"), so it was suppressed. This overturns Phase 28's "residual tap" conclusion: with `paintTo` zeroed the popup never drew anyway, and the log shows this `onTapClose` was a real tap 5s later, not a residual gesture. The earlier InfoMessage (no such word pair) and poker24 (no match) both fit. Fix: drop v1.0.12's delay/TapClose-absorb/diagnostics; after `UIManager:show(tv)`, if `_bookloadcover_suppressed` is set, restore `paintTo` (from `_original_paintTo_bookloadcover`), `visible`, `hasVisibleContent` and `setDirty` to force repaint (defensive: only if original paintTo is a function; branch is skipped when the patch isn't installed). SYNTAX OK.
